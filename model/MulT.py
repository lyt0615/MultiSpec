...
hyp_params = args
hyp_params.orig_d_l, hyp_params.orig_d_a, hyp_params.orig_d_v = 1, 1, 1
hyp_params.l_len, hyp_params.a_len, hyp_params.v_len = 1024, 512, 256
hyp_params.layers = args.nlevels
hyp_params.output_dim = 37


conv_param =[ 
            [{'conv_cin': 1, 'conv_cout': 32, 'conv_ksize': 5, 'conv_stride': 2, 'conv_padding': 1, 'mp_ksize': 2, 'mp_stride': 2,},
...
def get_output_size(input_size, conv_param, fc_layers):
    ...
    return {'input_dim': input_size[0]*input_size[1], 'num_layers': fc_layers}

# class Spec_Embedder(nn.Module):
#     def __init__(self, conv_param, class_num=37):
#         super(Spec_Embedder, self).__init__()
...
#         # fc layers
#         fc_layers = []
#         fc_param = get_output_size([1, X_train.shape[1]], conv_param, fc_layers)
#         input_dim= fc_param['input_dim']
#         output_dim = 1024
#         for i in range(fc_param['num_layers']):
#             if i != fc_param['num_layers']-1:
#                 fc_layers.extend([nn.Linear(input_dim, output_dim),
#                                 nn.ReLU(),
#                                 # nn.Dropout(),
#                                 ])
#                 if fc_param['dropout'] is not None:
#                     fc_layers.extend([nn.Dropout(fc_param['dropout'])])
#                 input_dim = output_dim
#                 output_dim = int(output_dim/2)
#             else:
#                 fc_layers.extend([nn.Linear(input_dim, class_num),
#                                 ])
...
class MULTModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        self.d_l, self.d_a, self.d_v = 64, 64, 64 # 30
        ...
        self.proj_l = nn.Conv1d(self.orig_d_l, self.d_l, kernel_size=8, stride=8, padding=0, bias=False)
        self.proj_a = nn.Conv1d(self.orig_d_a, self.d_a, kernel_size=8, stride=8, padding=0, bias=False)
        self.proj_v = nn.Conv1d(self.orig_d_v, self.d_v, kernel_size=8, stride=8, padding=0, bias=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_heads', type=int, default=5,
                        help='number of heads for the transformer network (default: 5)')
    parser.add_argument('--attn_mask', action='store_false', default=False,
                        help='use attention mask for Transformer (default: true)')
